...
public class State {
    ...
    int calculateMainFunction() {
        if (graph.redColor == 0 && graph.blueColor == 0) value = Integer.MIN_VALUE + 1;
        else if (graph.redColor == 0) value = Integer.MIN_VALUE;
        else if (graph.blueColor == 0) value = Integer.MAX_VALUE;
        else {
           if(Game.difficulty.getValue().equals("Easy")) value = graph.redColor;
