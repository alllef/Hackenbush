...
public class StateTree {
    ...
    Rib ribToRemove() {
        for (State tmp : state.stateList) {
            if (tmp.value == state.value) {

                for (Rib rib:state.graph.ribsList){
                    if(rib.firstDot==tmp.removedRib.firstDot&&rib.secondDot==tmp.removedRib.secondDot&&rib.color==tmp.removedRib.color) return rib;
                }
